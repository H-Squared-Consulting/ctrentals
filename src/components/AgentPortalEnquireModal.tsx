...
export default function AgentPortalEnquireModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    // properties.length === 0 is allowed — that's a "general
    // enquiry" (dates only, no specific houses named). Team-side
    // the deal lands in Arrived with no requested_property_ids and
    // the user picks properties manually when generating proposals.
    ...
    <form id="agent-enquire-form" onSubmit={submit}>
      ...
      >
        ...
        {/* Agent's own reference label — required, free text, shown
            ...
            the AHH/N code the team uses internally. Routing is
            automatic: every agent-portal enquiry is assigned to
            both Nicki + Hayley on the team side. */}
